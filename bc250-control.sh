...
#
# NOTE:
# - Defaults to /sys/class/drm/card1, but accepts BC250_CARD=cardN or --card cardN.
...
configure_paths() {
  case "$CARD" in
    card*[!0-9]*|card)
      echo "Invalid card name: $CARD"
      echo "Use a DRM card name such as card0 or card1."
      exit 2
      ;;
    card[0-9]*)
      ;;
  ...
  DEBUG_PM_INFO="/sys/kernel/debug/dri/${CARD_INDEX}/amdgpu_pm_info"
}
...
parse_args() {
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --card)
        [ -n "${2:-}" ] || {
          echo "--card requires a card name, for example card1"
          exit 2
        }
        CARD="$2"
        shift 2
